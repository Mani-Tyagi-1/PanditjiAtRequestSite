...
#### React SPA Audit Quick Check

For React/Vue/Angular SPAs, run these checks before deep analysis:

1. **Visible text test:** `curl -s [url] | python3 -c "import sys,re; html=sys.stdin.read(); body=re.search(r'<body[^>]*>(.*?)</body>', html, re.DOTALL); text=re.sub(r'<[^>]+>', '', body.group(1)).strip() if body else ''; print(f'{len(text)} chars visible')"` — if <200 chars, Googlebot sees nothing
2. **H1 count in HTML:** `grep -c '<h1'` on fetched HTML — should be >0 without JS
3. **Inline JSON-LD:** Check how many schema blocks are in the raw HTML vs injected by JS
4. **Route coverage:** Check for missing index routes (e.g., `/blog` detail route exists but `/blog` listing route missing) and catch-all redirects that hide 404s
5. **Script bundle size:** Single large JS bundle = CSR indicator

See `references/react-spa-seo.md` for full diagnostic commands and fix priority order.
