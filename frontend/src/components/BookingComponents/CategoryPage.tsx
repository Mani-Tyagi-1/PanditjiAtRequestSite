...
export default function CategoryPage() {
    ...
    // Dynamically add serviceName to categories if it's not present so the tab shows
    const categoryList = serviceName && !CATEGORIES.includes(serviceName)
        ? [serviceName, ...CATEGORIES]
        : CATEGORIES;

    const [activeCategory, setActiveCategory] = useState<string>(serviceName || "Wealth");
    ...
    const pujas = PUJAS[activeCategory] || [];
    const recommended = pujas.filter((p) => p.popular);
    const all = pujas.filter((p) => !p.popular);
                    ...
                    <div className="flex gap-2.5 overflow-x-auto scrollbar-hide pb-2">
                        {categoryList.map((cat) => (
                            <button
                                ...
                                className={`shrink-0 text-sm font-semibold px-4 py-1 rounded-full border transition-all duration-200 ${activeCategory === cat
                ...
                {all.length > 0 && (
                    <div className="px-4 pb-10 fade-up" key={`all-${activeCategory}`} style={{ animationDelay: "60ms" }}>
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <span className="text-xs font-semibold text-stone-500 bg-stone-100 px-2.5 py-1 rounded-md">{pujas.length} Services</span>
                        </div>
