  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        // optional cache
        sessionStorage.setItem("pooja_categories", JSON.stringify(activeCategories));
      } catch (error) {
        ...
        <div className="w-full max-w-md px-4">
          <div className="flex items-center justify-center mb-4">
            ...
            <div
              ...
              style={{
                backgroundImage:
                  'url("https://vedic-vaibhav.blr1.cdn.digitaloceanspaces.com/Pandit%20ji%20at%20request/gradientBox.png")',
              }}
            >
              Puja Services
            </div>
          ...
          <div className="grid grid-cols-4 sm:grid-cols-4 gap-2">
            {isLoading ? (
              Array.from({ length: 8 }).map((_, idx) => (
                <div
                  key={idx}
                  className="bg-white/50 animate-pulse rounded-2xl border border-orange-100 p-1 flex flex-col items-center h-24"
                />
              ))
            ) : categories.length > 0 ? (
                ...
                >
                  <div className="w-12 h-12 flex items-center justify-center">
                    <img
                      ...
                      onError={(e) => {
                        e.currentTarget.src =
                          "https://vedic-vaibhav.blr1.digitaloceanspaces.com/vedic-vaibhav/category-images/category-images_1771234666851.png";
                      }}
                    />
                  </div>
