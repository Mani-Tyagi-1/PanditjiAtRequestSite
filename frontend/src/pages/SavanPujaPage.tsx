...
const PLACEHOLDER_REVIEWS: Review[] = [
    ...
    { name: "Rakesh Kumar", rating: 5, date: "3 weeks ago", text: "₹501 mein Kashi Vishwanath se puja karwana bahut easy tha. Sab update WhatsApp pe mila.", verified: true },
...
// ── Small UI pieces (Savan palette: light emerald/green + saffron) ────
...
function Accordion({ title, icon, defaultOpen = false, children }: {
    ...
    const panelId = useId();
    return (
        <div className={`bg-white border rounded-2xl overflow-hidden transition-colors ${open ? "border-emerald-300" : "border-emerald-100"}`}>
            <button
                type="button"
                aria-expanded={open}
                ...
                className="w-full px-3.5 py-3 flex items-center justify-between text-left focus-visible:ring-2 focus-visible:ring-emerald-400 outline-none"
            >
                <span className="flex items-center gap-2 text-[14px] font-bold text-stone-800">
                    {icon}{title}
                </span>
                <ChevronDown className={`w-4 h-4 text-emerald-500 shrink-0 transition-transform ${open ? "rotate-180" : ""}`} />
            </button>
            <div id={panelId} hidden={!open} className="px-3.5 pb-3.5 pt-1 text-[12.5px] text-stone-600 leading-relaxed border-t border-emerald-50">
                {children}
            </div>
        </div>
    );
...
function SectionTitle({ icon, children }: { icon?: React.ReactNode; children: React.ReactNode }) {
    return (
        <h3 className="flex items-center gap-1.5 text-[12.5px] font-extrabold uppercase tracking-wider text-emerald-700 mb-2.5">
            {icon}{children}
        </h3>
    );
}
    ...
    return (
        <div className="overflow-hidden -mx-4 px-4">
            ...
            <div className="review-track flex gap-2.5">
                {items.map((r, i) => (
                    <div key={i} className="shrink-0 w-56 bg-white border border-emerald-100 rounded-xl p-3 shadow-sm">
...
// Varanasi on the first Savan Somwar. Structurally mirrors DurgaMataPujaPage
// but carries a distinct Savan/Shiv theme (light emerald-green + saffron, abhishek
...
export default function SavanPujaPage() {
    ...
    // taps the CTA on the booking page.
    const openBooking = () => navigate(`/${KASHI_MAHADEV_PUJA_SLUG}/booking`);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-emerald-50 via-[#F2FBF6] to-teal-50/70 pb-24 font-sans w-full max-w-md mx-auto shadow-xl relative border-x border-emerald-100">
        ...
        {/* ── Sticky header ── */}
        <div className="sticky top-0 z-50 bg-[#F2FBF6]/90 backdrop-blur-md border-b border-emerald-100 px-4 py-3 flex items-center gap-3">
          <button
            onClick={() =>
              location.key !== "default" ? navigate(-1) : navigate("/")
            }
            aria-label="Go back"
            className="w-8 h-8 rounded-full bg-white flex items-center justify-center border border-emerald-200/60 shadow-sm active:scale-90 transition-transform"
          >
          ...
          <button
            ...
            disabled={isSharing}
            aria-label="Share"
            className="w-8 h-8 rounded-full bg-white flex items-center justify-center border border-emerald-200/60 shadow-sm active:scale-90 transition-transform disabled:opacity-60"
          >
            {shareCopied ? (
        ...
        {/* ── Hero banner (dark Shiv night band + abhishek droplets) ── */}
        ...
        <div className="px-4 pt-3 pb-4 space-y-4">
          ...
          <div>
            ...
            <div className="flex flex-col gap-1 mt-2 text-[12px] text-stone-600">
              <span className="flex items-start gap-1.5">
                <Mountain className="w-3.5 h-3.5 text-emerald-500 shrink-0 mt-0.5" />
          ...
          {/* ── Hero value props ── */}
          <div className="rounded-2xl border border-emerald-100 bg-gradient-to-br from-white via-emerald-50/50 to-teal-50/60 p-3.5 shadow-sm">
          ...
          {/* ── Countdown to the puja date ── */}
          <div className="flex flex-col items-center gap-2 bg-white border border-emerald-100 rounded-xl px-3 py-2.5 shadow-sm">
            <span className="text-[11px] font-bold text-emerald-700 leading-tight text-center">
              ...
              <div className="flex items-center justify-center gap-1.5">
                {[
                  ...
                  <div key={u.l} className="flex items-center gap-1.5">
                    <div className="min-w-[40px] bg-emerald-50 border border-emerald-100 rounded-lg px-1.5 py-1 text-center">
                      <div className="text-[16px] leading-none font-bold text-emerald-900 tabular-nums">
                        {pad2(u.v)}
                      </div>
          ...
          <div>
            <SectionTitle icon={<Droplets className="w-3.5 h-3.5 text-emerald-400" />}>
            ...
            <div className="grid grid-cols-4 gap-2">
              {offerings.map(({ icon: Icon, label, sub }) => (
                <div
                  key={label}
                  className="bg-white border border-emerald-100 rounded-xl p-2 text-center shadow-sm"
                >
                  <div className="w-7 h-7 mx-auto rounded-full bg-gradient-to-br from-emerald-100 to-teal-200/70 flex items-center justify-center mb-1">
                    <Icon className="w-3.5 h-3.5 text-emerald-700" />
                  </div>
                  <p className="text-[10.5px] font-bold text-stone-800 leading-tight">{label}</p>
                  <p className="text-[8.5px] text-stone-400 leading-tight mt-0.5">{sub}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* ── Mantra strip ── */}
          <div className="rounded-2xl bg-gradient-to-r from-emerald-700 via-green-600 to-emerald-700 px-4 py-3 text-center shadow-md">
          ...
          <div>
            <SectionTitle icon={<Star className="w-3.5 h-3.5 text-amber-400" />}>
              Loved by devotees
            </SectionTitle>
          ...
          {/* ── How it works ── */}
          <div className="rounded-2xl border border-emerald-100 bg-gradient-to-br from-white via-emerald-50/50 to-teal-50/60 p-3.5 shadow-sm">
            <SectionTitle icon={<Sparkles className="w-3.5 h-3.5 text-emerald-400" />}>
          ...
          <div className="rounded-2xl border border-emerald-100 bg-gradient-to-br from-white via-emerald-50/50 to-teal-50/60 p-3 shadow-sm">
            <SectionTitle icon={<Sparkles className="w-3.5 h-3.5 text-emerald-400" />}>
              Why perform this puja
            </SectionTitle>
          ...
          <div>
            <SectionTitle icon={<Flame className="w-3.5 h-3.5 text-emerald-400" />}>
              Puja details
            </SectionTitle>
          ...
          {/* ── What you'll get ── */}
          <div>
            <SectionTitle icon={<Gift className="w-3.5 h-3.5 text-emerald-400" />}>
              What you'll get
            </SectionTitle>
            <div className="grid grid-cols-3 gap-2">
              {whatYouGet.map(({ icon: Icon, title, sub }) => (
                <div
                  key={title}
                  className="bg-white border border-emerald-100 rounded-xl p-2.5 text-center shadow-sm"
                >
                  <div className="w-8 h-8 mx-auto rounded-full bg-gradient-to-br from-emerald-100 to-teal-200/70 flex items-center justify-center mb-1.5">
          ...
          {puja.faqs.length > 0 && (
            <div>
              <SectionTitle icon={<HelpCircle className="w-3.5 h-3.5 text-emerald-400" />}>
          ...
          {/* ── Footer / ecosystem ── */}
          <footer className="pt-3 mt-2 border-t border-emerald-100 text-[11px] text-stone-500 space-y-2">
        ...
        {/* ── Sticky bottom CTA ── */}
        <div className="fixed bottom-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-t border-emerald-100 max-w-md mx-auto shadow-lg">
          <div className="px-4 pt-2 pb-2.5">
            <div className="flex items-center gap-3">
              <div className="shrink-0">
                ...
                <span className="text-[19px] font-extrabold text-emerald-700">
                  ₹{price.toLocaleString("en-IN")}
                </span>
              </div>
              <button
                onClick={openBooking}
                className="flex-1 bg-gradient-to-r from-emerald-700 via-green-600 to-emerald-700 text-white font-bold text-[15px] py-3 rounded-xl shadow-md active:scale-95 transition-all focus-visible:ring-2 focus-visible:ring-emerald-400 outline-none"
              >
