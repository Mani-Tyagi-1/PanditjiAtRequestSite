...
const COD_ENV_AVAILABLE =
  typeof process !== "undefined"
    ? process.env.NEXT_PUBLIC_COD_AVAILABLE === "true" || process.env.VITE_COD_AVAILABLE === "true"
    : false;
const COD_ENV_MINIMUM = Math.max(
  0,
  typeof process !== "undefined"
    ? Number(process.env.NEXT_PUBLIC_MINIMUM_COD_AMOUNT || process.env.VITE_MINIMUM_COD_AMOUNT) || 0
    : 0
);
                                                ...
                                                <div key={l.product._id} className="flex gap-3 bg-white p-3 rounded-2xl border border-orange-50 shadow-xs">
                                                    <div className="w-16 h-16 bg-orange-50/20 rounded-xl border border-orange-100 overflow-hidden shrink-0">
                                                        {l.product.featuredImage?.url ? (
                                                            <img loading="lazy" decoding="async" src={l.product.featuredImage.url} alt={l.product.title} className="w-full h-full object-cover" />
                                                        ) : (
