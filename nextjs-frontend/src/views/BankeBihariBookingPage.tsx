...
function resolveBookingDate(dateLabel: string, time: string): string {
    // The puja happens in IST, so pin the instant to IST explicitly instead of
    // the browser's timezone (which shifts the stored date/time for overseas
    // devotees, and can flip the calendar day via toISOString).
    const base = new Date(dateLabel);
    if (isNaN(base.getTime())) return new Date().toISOString();
    const m = time?.match(/(\d{1,2}):(\d{2})/);
    const hh = m ? Math.min(23, parseInt(m[1], 10)) : 0;
    const mm = m ? Math.min(59, parseInt(m[2], 10)) : 0;
    const p = (n: number) => String(n).padStart(2, "0");
    const iso = `${base.getFullYear()}-${p(base.getMonth() + 1)}-${p(base.getDate())}T${p(hh)}:${p(mm)}:00+05:30`;
    const out = new Date(iso);
    return isNaN(out.getTime()) ? base.toISOString() : out.toISOString();
}
...
export default function BankeBihariBookingPage() {
    ...
    const handleConfirm = async (skipRecommendations = false, checkout?: { shopSubtotal: number; relatedPuja?: RecommendedPuja | null }) => {
        ...
        if (isIndia && form.email.trim() && !/^\S+@\S+\.\S+$/.test(form.email.trim())) {
            ...
            const rzp = new RazorpayCtor({
                ...
                handler: async (response: any) => {
                    try {
                        ...
                        setError((verifyErr.message || "Payment verification failed.") + " If money was deducted, please contact support on WhatsApp instead of paying again.");
                    } finally {
 ...
 setSubmitting(false);
    ...
    useEffect(() => {
        ...
        // Flag is set when the timer actually fires (not at schedule time) so a
        // StrictMode effect re-run, which cancels the first timer, still resumes.
        const timer = window.setTimeout(() => { resumedCheckoutRef.current = true; void handleConfirm(true); }, 350);
