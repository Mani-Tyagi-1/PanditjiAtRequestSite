...
export const createPendingBooking: RequestHandler = async (req, res, next) => {
  try {
    ...
    if (!userExists) { res.status(404).json({ success: false, message: 'User not found.' }); return; }
    if (!poojaExists) { res.status(404).json({ success: false, message: 'Pooja not found.' }); return; }
...
export const completePoojaBooking: RequestHandler = async (req, res, next) => {
  try {
    ...
    if (!pendingDoc) {
      res.status(404).json({ success: false, message: 'Pending booking not found.' });
      return;
    }
    ...
    if (pendingDoc.amount !== amountPaid) {
      res.status(400).json({ success: false, message: 'Amount paid mismatch. Potential fraud or error.' });
      return;
    }
...
export const getPendingBookingsByUserPhone: RequestHandler = async (req, res, next) => {
  try {
    ...
    if (!userPhone) {
      res.status(400).json({ success: false, message: 'User phone number is required.' });
      return;
    }
...
export const getPendingBookingsForPandit: RequestHandler = async (req, res, next) => {
  try {
    ...
    if (!panditId) { res.status(400).json({ success: false, message: 'panditId is required.' }); return; }
...
export const getOnePendingBooking: RequestHandler = async (req, res, next) => {
  try {
    ...
    if (!bookingId) { res.status(400).json({ success: false, message: 'bookingId required' }); return; }
