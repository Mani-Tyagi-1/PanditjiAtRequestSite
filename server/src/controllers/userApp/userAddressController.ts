// src/controllers/userApp/userAddressController.ts
import { Request, Response, NextFunction, RequestHandler } from "express";
import userAddressModel from "../../models/userApp/userAddressModel";
import servicabileCityModel from "../../models/userApp/servicabileCityModel";
...
export const getAddressById: RequestHandler = asyncHandler(async (req, res) => {
  ...
  if (!userId) return sendEncryptedResponse(res, 400, { message: "User ID is required (body.userId, query.userId or x-user-id header)." });
  if (!isValidObjectId(userId)) return sendEncryptedResponse(res, 400, { message: "Invalid user ID." });
  if (!isValidObjectId(id)) return res.status(400).json({ success: false, message: "Invalid address ID." });

  const address = await userAddressModel.findOne({ _id: id, user: userId });
  if (!address) return res.status(404).json({ success: false, message: "Address not found or does not belong to the user." });

  return res.status(200).json({ success: true, message: "Address fetched successfully.", data: address });
});
