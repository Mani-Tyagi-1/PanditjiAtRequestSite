import WhatsappSession from '../model/whatsapp/WhatsappSession.model';
import WhatsappBooking from '../model/whatsapp/WhatsappBooking.model';
import { sendTextMessage } from './pinnacle.service';
...
function buildConfirmationSummary(data: Record<string, any>): string {
  return (
    `Kripya details confirm karein:\n\n` +
    `*Puja:* ${data.pujaName}\n` +
    `*Name:* ${data.name}\n` +
...
async function saveBooking(phone: string, data: Record<string, any>): Promise<void> {
  try {
    const booking = await WhatsappBooking.create({
      source: 'whatsapp',
      phone,
      name: data.name,
      gotra: data.gotra || '',
    ...
    await sendTextMessage(
      phone,
      `Aapki booking request receive ho gayi hai 🙏\n\n` +
    ...
    await sendTextMessage(
      phone,
      `Khed hai, booking save karne mein kuch samasya aa gayi. ` +
...
export async function handleIncomingMessage(phone: string, messageText: string): Promise<void> {
  ...
  if (!session) {
    session = new WhatsappSession({ phone, step: 'START', data: {}, lastMessage: '' });
  }
  ...
  if (input === 'menu' || input === 'main menu') {
    ...
    await session.save();
    await sendTextMessage(phone, MAIN_MENU);
    return;
  }
  ...
  switch (session.step) {
    ...
    case 'MAIN_MENU': {
      if (input === '1' || input.includes('book') || input.includes('puja')) {
        ...
        await session.save();
        await sendTextMessage(phone, PUJA_MENU);
      } else if (input === '2' || input.includes('baat') || input.includes('pandit')) {
        await sendTextMessage(
          phone,
          `Pandit Ji se baat karne ke liye hamare helpline par call karein.\n\n` +
            `"menu" type karein wapas jaane ke liye.`
        );
      } else if (input === '3' || input.includes('price') || input.includes('dakshina')) {
        await sendTextMessage(
          phone,
          `Puja prices ke liye hamari website visit karein ya helpline par call karein.\n\n` +
            `"menu" type karein wapas jaane ke liye.`
        );
      } else if (input === '4' || input.includes('check') || input.includes('booking status')) {
        await sendTextMessage(
          phone,
          `Booking status check karne ke liye helpline par call karein.\n\n` +
            `"menu" type karein wapas jaane ke liye.`
        );
      } else {
        await sendTextMessage(
          phone,
          `Kripya sahi option chunein:\n\n` +
            `1️⃣ Puja Book Karni Hai\n` +
            `2️⃣ Pandit Ji Se Baat Karni Hai\n` +
            `3️⃣ Puja Price Dekhna Hai\n` +
            `4️⃣ Apni Booking Check Karni Hai\n\n` +
    ...
    // ── PUJA SELECTION ───────────────────────────────────────────────────────
    case 'ASK_PUJA': {
      let pujaName: string | null = null;

      if (input === '1') pujaName = PUJA_LIST[0];
      else if (input === '2') pujaName = PUJA_LIST[1];
      else if (input === '3') pujaName = PUJA_LIST[2];
      else if (input === '4') pujaName = PUJA_LIST[3];
      else if (input === '5') pujaName = PUJA_LIST[4];
      else {
        // Allow user to type puja name directly
        const matched = PUJA_LIST.find((p) => input.includes(p.toLowerCase()));
        if (matched) pujaName = matched;
      }
    ...
    case 'ASK_NAME': {
      ...
      if (name.length < 2) {
        await sendTextMessage(phone, `Kripya apna poora naam likhein (kam se kam 2 akshar).`);
        break;
      }
    ...
    case 'ASK_GOTRA': {
      ...
      await session.save();
      await sendTextMessage(phone, `Aap kis *city* mein hain?\n(Apne sheher ka naam likhein)`);
      break;
    }
    ...
    case 'ASK_CITY': {
      ...
      if (city.length < 2) {
        await sendTextMessage(phone, `Kripya apne sheher ka naam likhein.`);
        break;
      }
    ...
    case 'ASK_DATE': {
      ...
      if (preferredDate.length < 3) {
        await sendTextMessage(
          phone,
          `Kripya preferred date likhein (jaise: 15 June 2025).`
        );
        break;
      }
    ...
    case 'ASK_MODE': {
      ...
      if (input === '1' || input.includes('home') || input.includes('ghar')) mode = 'Home';
      else if (input === '2' || input.includes('temple') || input.includes('mandir')) mode = 'Temple';
      else if (input === '3' || input.includes('online')) mode = 'Online';
      ...
      if (mode) {
        ...
        await session.save();
        await sendTextMessage(phone, buildConfirmationSummary(session.data));
      } else {
        await sendTextMessage(
          phone,
    ...
    case 'CONFIRM_BOOKING': {
      const isConfirm =
        ...
        input.includes('ok');
      const isEdit =
        input === '2' || input.includes('edit') || input.includes('change') || input.includes('nahi');
      ...
      if (isConfirm) {
        ...
        session.markModified('data');
        await session.save();
        await sendTextMessage(phone, `Theek hai! Aao dobara shuru karte hain. 🙏\n\n${PUJA_MENU}`);
      } else {
        await sendTextMessage(
          phone,
          `Kripya sirf *1* (Confirm) ya *2* (Edit) type karein.\n\n${buildConfirmationSummary(session.data)}`
        );
      }
      break;
    }
